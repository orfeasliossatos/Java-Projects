...
public abstract class Area implements Playable {
    ...
    private void updateCamera () {
    	...
    	// The camera follows the player until the player close than followDistanceFromSides from the side of the area.
    	int followDistanceFromSides = 5; 
    	
    	// Update expected viewport center
    	if (viewCandidate == null) {
    		viewCenter = new Vector(getWidth() / (float)2, getHeight() / (float)2);
    	} else {
    		float newViewCenterX = viewCandidate.getPosition().x;
    		float newViewCenterY = viewCandidate.getPosition().y;
    		
    		if (viewCandidate.getPosition().x < followDistanceFromSides) {
    			newViewCenterX = followDistanceFromSides;    			
    		}
    		
    		if (viewCandidate.getPosition().y < followDistanceFromSides) {
    			newViewCenterY = followDistanceFromSides;
    		}
    		
    		if (viewCandidate.getPosition().x > getWidth() - followDistanceFromSides) {
    			newViewCenterX = getWidth() - followDistanceFromSides;
    		}
    		
    		if (viewCandidate.getPosition().y > getHeight() - followDistanceFromSides) {
    			newViewCenterY = getHeight() - followDistanceFromSides;
    		}
    		viewCenter = new Vector(newViewCenterX, newViewCenterY);
    	}
